...
> The .cpg file would contain character encoding info for text fields to display correctly. The .dbf is a table for each feature and the .prj is a coordinate reference system to describe the map projection. The .shp and .shp.xml files contain the shapes of the data and provide documentation about the dataset. Finally, the .shx file links the .shp file to the .dbf file. 
...
> All the counties printed and "TRUE" is representing that all counties exist, except there are a few duplicates like "Colusa". 
...
> This means that the shapfile has been read and R is telling the spatial extent of the data, specifically the coordinates. 
...
> The projection allows us to represent the curved surface of the earth on a flat map. It tells us how the geographic data are represented, what units the coordinates use, and how spatial relationships should be interpreted.
...
ggplot() +
  geom_sf() +
  labs(title = "Northernmost California Counties Bordering Oregon")
map1
```
...
## Using the mortality dataset containing the total number of deaths by county, year, and month
mortality_sf <- totalDeaths %>% 
  ## Rename the County column to COUNTY_NAM so it matches the county name column in the California shapefile
  rename(COUNTY_NAM = County) %>%
  ## Group the data by county, year, and month so deaths can be summarized for each location and time period
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## Calculate the total number of deaths for each county, year, and month, ignoring missing values
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## Join the mortality data with the California county shapefile using the COUNTY_NAM field.
  ## Relationship = "many-to-many" allows multiple mortality records (e.g., different months/years) to matchthe same county boundary in the shapfile.
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Convert the joined data back into an sf (spatial) object so it can be mapped with geom_sf()
  st_as_sf()
```
...
> st_join performs a spatial join, it joins datasets based on their geographic relationships. We could not substitute st_join in this case because mortality data does not contain geographic coordinates or geometries. 
...
> Mortality_sf contains mutliple records for each county where as cali_sf only has one entry per county. 
...
> There are 58 distinct counties and earlier we found how many rows were present in the dataset.
...
> Yes, the map of total deaths is heavily influenced by population size which makes it difficult to compare mortality across counties. 
...
key <- "5316bb9bca2692aa9efa07830e47ea5e236a21fb"
```
...
> `DP1_0048C` expresses the count or number of people and `DP1_0048P`reports the percentage of the population in that same category.
...
> It breaks down columns based on each gender and then also groups each gender aged 65 and up in each county.
...
> To work with a different API we'd have to change the parts of the function specific to the Census API such as the base URL and query parameters. Overall, the workflow would remain generally the same such as retrieving the data, cleaning and processing, removing unneeded information, and formatting. 
...
mortality_sf2 <- mortality_sf %>%
  full_join(census, by = c("COUNTY_NAM" = "County")) %>%
  st_as_sf()
```
...
> Now that we've adjusted for population size, the pattern changes considerably. Los Angeles is no longer the darkest, counties such as Shasta are mugh more accurately represented. This doesn't necessarily tell us about COVID-19 cases becasue this map represents all-casue mortality.
...
> This happened becasue we are only seeing 8 months of data instead of 12. One option is to compare monthly mortality rates over the years rather than annual totals. 
...
> After adjusting to use the average monthly percent mortality, the 2020 map appears much more consistent with what would be expected during the COVID-19 pandemic. Because the calculation no longer depends on having a full 12 months of data, the 2020 mortality estimates are no longer artificially low. Several counties, including Los Angeles, Orange, Riverside, San Diego, and Imperial, show higher mortality percentages than in previous years most likely due to the pandemic.
...
> The regression predicitons are not completely robust because they depend on the assumptions of a linear regression model.
