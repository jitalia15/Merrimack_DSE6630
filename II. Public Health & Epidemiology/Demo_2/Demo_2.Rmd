...
> The group variable is necessary because it tells ggplot which observations belong to each state boundary, preventing unrelated coordinate points from being connected across different states. 
...
> The fully-processed data from Project 1 was not used becasue many preprocessing steps were performed and removed/altered information needed for spatial analysis. This suggests that the best dataset choice differs by ananlytical goal. Machine learning models like elastic net often benefit from heavily processed, standardized, and encoded data to optimize prediction performance. Spatial analyses require more raw, minimally modified datasets that retain geographic structure and original variable meaning.
...
> The region column depicts each state fully spelled out in lowercase and the pneumonia dataset uses capitalized abbreviations. 
...
pneumoniaFull <- pneumoniaFull %>%
  left_join(state.fips[, c("abb", "polyname")],
            by = c("State" = "abb")) %>%
  mutate(State = polyname) %>%      # replace abbreviations with full names
  select(-polyname)                 # remove extra column
```
...
> To fix this, we could mdofy state.flips to keep only one unique mapping per state abbreviation. However, at the county level this would be much more tedious and error-prone than doing so for the 50 states. 
...
# New "feature" column
pneumoniaFull <- pneumoniaFull %>%
  mutate(feature = getStateName(State))
```
...
```{r}
 stateAggPneumonia <- pneumoniaFull %>%
   ...
   select(PredictedReadmissionRate, feature, 
          `Score_Death rate for pneumonia patients`, 
          `Score_Hospital return days for pneumonia patients`, 
          `Score_Medicare spending per patient`) %>%  
   ## Change region to a factor
   mutate(region = as.factor(feature)) %>% 
   ## Group by region
   group_by(region) %>%  
...
> My code did not load in properly, the columns populated but there were now rows of data. I think this is becasue my states were abbreviated anf they were meant to be spelled out. 
...
> Showing a mean when the data is heavily skewed, or a median when stakeholders are trying to see the whole picture, could substantially distort the understanding of the data and downstream decisions. Selecting the appropriate summary statistic depends on the distribution of the data and the analytic goal. 
...
> Medicare spending per patient, pneumonia hospital return days, and pneumonia readmission rate could all benefit from the mediam over the mean. They are heavily skewed with outliers far outside the main distribution. Peumonia death rate is not as skewed but does contain many outliers.
...
___Interpretation___: Because $p<0.05$, we reject the hypothesis of symmetry and conclude that pneumonia death dates are __significantly skewed__. We should use the median to aggregate this variable.

```{r}
agostino.test(pneumoniaFull$`Score_Hospital return days for pneumonia patients`)
```

___Interpretation___: Because $p<0.05$, we reject the hypothesis of symmetry and conclude that pneumonia hospital return days are __significantly skewed__. We should use the median to aggregate this variable.

```{r}
agostino.test(pneumoniaFull$`Score_Medicare spending per patient`)
```

___Interpretation___: Because $p<0.05$, we reject the hypothesis of symmetry and conclude that medicare costs per patient are __significantly skewed__. We should use the median to aggregate this variable.
...
stateAggPneumonia <- stateAggPneumonia %>%
  select(
    -meanReadmissions,
    -medianDeathRate,
    -meanHospitalReturnDays,
    -meanMedicareSpending
  )
```
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
 ...
 ## !!Update color here   
  ...
  labs(title = "Median predicted % readmissions within 30 days",
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Mean predicted % death rate within 30 days", 
            low = "white", high = skittles[3]) + 
 ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Mean predicted % of Pneumonia death rates within 30 days",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median predicted % Medicare spending within 30 days", 
            low = "white", high = skittles[7]) +
 ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median predicted % of Medicare spending within 30 days",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
  ## -- This makes the map pretty. No need to change any of this -- ##
  ## Makes a completely blank map background
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median predicted % hopsital return days within 30 days", 
            low = "white", high = skittles[5]) +
 ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median predicted % hopsital return days within 30 days",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
  ## -- This makes the map pretty. No need to change any of this -- ##
  ## Makes a completely blank map background
