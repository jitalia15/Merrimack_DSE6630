...
> I chose the fully cleaned and encoded dataset because the work has been done to see the observed readmission rate. Plus, other values have already been cleaned up. 
...
longTargets %>% 
  ggplot(aes(x = Value, y = NatlComparisonHospitalDays, fill = NatlComparisonHospitalDays)) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.1) +  # Overlay boxplot quartiles on violin plot
  labs(title = "Which target to use?",
       subtitle = "By readmitted LOS in hospital compared to national average",
        x = "",
        y = "Rate or Score",
...
> The observed readmission rate compared to the national average is most distinct to me with minimal overlap between better, same, and worse.The progression of values depicted in the violin plot also suggest they relatively align with the national average. 
...
> This view confirms my decision from above! The groups are clearly ordered from left to right with a visible shift in the distributions. 
...
longScore %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of Observed pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
longHCAHPS %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                ...
                show.legend = T) + 
    ## CHANGE THE TITLE FOR FULL CREDIT!
    labs(title = "Relationship of _fill in this blank_ pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
        x = "Rate or Score",
        y = "Predicted Pneumonia-related Readmissions",
        color = "Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
...
```{r}
# ind <- createDataPartition(dat2Analyze$___,   ## Put the target here!
#                            p = ___,           ## Use the object you just made!
#                            list = FALSE)
# 
# train <- dat2Analyze[___, ]
